...
```
CLASE Product                    OBJETO (instancia)
┌─────────────────┐              ┌─────────────────┐
│ nombre           │   new  →    │ nombre = "Teclado"│
│ precio           │             │ precio = 25000.0  │
│ stock            │             │ stock = 15         │
│ describir()      │             └─────────────────┘
│ calcularValor()  │             ┌─────────────────┐
└─────────────────┘             │ nombre = "Mouse"    │
   (el molde)                    │ precio = 8000.0     │
                                  │ stock = 40           │
                                  └─────────────────┘
...
Cuando creás un objeto, Java lo pone en dos zonas distintas de memoria:

```
STACK (pila)                          HEAP (montón)
─────────────                         ─────────────
teclado ─────────────┐                ┌──────────────────┐
  (referencia, 8B)   │    ───────→   │ Product {         │
                      │               │   nombre="Teclado" │
mouse ────────────────┼──────→       │   precio=25000.0   │
  (referencia, 8B)   │               │   stock=15         │
                      │               └──────────────────┘
                      │               ┌──────────────────┐
                      └──────────────→│ Product {         │
                                      │   nombre="Mouse"   │
...
- **Stack**: guarda las variables locales y las REFERENCIAS (flechas) a los objetos. Es rápido y automático — cuando un método termina, sus variables desaparecen del Stack.
- **Heap**: guarda los OBJETOS mismos (los datos reales). El Garbage Collector limpia los objetos que ya nadie referencia.

Esto explica por qué `Product teclado = new Product(...)` NO guarda el producto entero en la variable — `teclado` es solo una FLECHA que apunta al objeto real en el Heap. Cuando pasás `teclado` como argumento a un método, pasás la FLECHA, no una copia del objeto entero.

Vas a profundizar esto en la Etapa 6 (JVM y memoria), pero es importante que desde el día 1 entiendas que los objetos SON referencias.
...
Todo objeto en Java pasa por tres etapas:

```
  new Product(...)      métodos, uso         sin referencias
  ───────────────→     ─────────────→       ───────────────→
   CREACIÓN             USO ACTIVO           GARBAGE COLLECTION
   (constructor)        (llamadas a          (la JVM lo limpia
    inicializa estado)   métodos, getters)    automáticamente)
```

1. **Creación**: `new` reserva memoria en el Heap y ejecuta el constructor.
2. **Uso**: tu código interactúa con el objeto a través de sus métodos.
3. **Recolección**: cuando ya NADIE tiene una referencia al objeto (la variable salió de scope, o la sobrescribiste con `null`), el Garbage Collector de la JVM lo detecta y libera esa memoria automáticamente — vos no tenés que hacer nada.

Esto es radicalmente distinto a lenguajes como C, donde tenés que liberar la memoria manualmente (`free()`). En Java, el GC se encarga.
